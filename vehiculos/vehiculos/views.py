...
def registro_vehiculos(request):
    ...
    contenedor = {'no_vehiculos': no_vehiculos, 'vehiculos': vehiculos, 'nums': nums}
    return render(request, 'registro_vehiculos.html', contenedor)

def vehiculos_buscados(request):
    if request.method == 'POST':
        buscado = request.POST.get('buscado', False)
        vehiculos_b = Vehiculo.objects.filter(placa__contains=buscado).order_by('-fecha_in')
        no_vehiculos = vehiculos_b.count()
        contenedor = {'buscado': buscado, 'vehiculos_b': vehiculos_b, 'no_vehiculos': no_vehiculos}
        return render(request, 'vehiculos_buscados.html', contenedor)
